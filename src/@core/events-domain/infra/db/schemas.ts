...
export const CustomerSchema = new EntitySchema<Customer>({
  ...
  properties: {
    id: { primary: true, type: new CustomerIdSchemaType() },
...
export const EventSchema = new EntitySchema<Event>({
  class: Event,
  properties: {
    id: { primary: true, type: new EventIdSchemaType() },
    name: { type: 'string', length: 255 },
    description: { type: 'text', nullable: true },
    date: { type: 'date' },
    is_published: { type: 'boolean', default: false },
    total_spots: { type: 'numeric', default: 0 },
    total_spots_reserved: { type: 'numeric', default: 0 },
    sections: {
      reference: '1:m',
      entity: () => EventSection,
